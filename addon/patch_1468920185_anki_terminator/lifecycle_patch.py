...
def is_called_from_target_addon():
    try:
        frame = sys._getframe(1)
        while frame:
            module_name = frame.f_globals.get("__name__", "")
            if module_name.startswith("1468920185") or module_name.startswith("1448033349"):
                return True
            frame = frame.f_back
    except Exception:
        pass
...
def is_called_from_restore_clipboard():
    try:
        frame = sys._getframe(1)
        while frame:
            if frame.f_code.co_name == "restore_clipboard":
                return True
            frame = frame.f_back
    except Exception:
        pass
...
def clone_mime_data(mime):
    ...
    try:
        ...
        companion_logger.log(f"[Clipboard Patch] Error cloning mime data: {e}")
    return None

def create_private_mime_data(text):
    mime = QMimeData()
    mime.setText(text)
    try:
        # Linux (Klipper, CopyQ, GPaste, etc.)
        mime.setData("x-kde-passwordManagerHint", QByteArray(b"secret"))
        
        # Windows / Office Clipboard / general hints
        mime.setData("ExcludeClipboardContentFromMonitorProcessing", QByteArray(b"true"))
        mime.setData("ClipboardViewerIgnoreFormat", QByteArray(b"true"))
        
        # macOS
        mime.setData("org.nspasteboard.TransientType", QByteArray(b"true"))
        mime.setData("org.nspasteboard.ConcealedType", QByteArray(b"true"))
        mime.setData("org.nspasteboard.AutoGeneratedType", QByteArray(b"true"))
    except Exception as e:
        companion_logger.log(f"[Clipboard Patch] Error setting privacy hints on QMimeData: {e}")
    return mime

def schedule_clipboard_restore(ai_text, backup_mime):
    def restore_clip():
        try:
            clipboard = QApplication.clipboard()
            if clipboard and clipboard.text() == ai_text:
                if backup_mime:
                    clipboard.setMimeData(backup_mime)
                    companion_logger.log("[Clipboard Patch] Restored previous clipboard content.")
                else:
                    clipboard.clear()
                    companion_logger.log("[Clipboard Patch] Cleared clipboard (no backup found).")
        except Exception as e:
            companion_logger.log(f"[Clipboard Patch] Error restoring clipboard: {e}")
            
    QTimer.singleShot(1500, restore_clip)
...
def patch(dock_web_view_mod):
    ...
    # Hook QClipboard to backup and restore previous clipboard content when target addon copies
    if not hasattr(QClipboard, "_companion_patched"):
        original_setText = QClipboard.setText
        def patched_setText(self, text, mode=None):
            cfg = mw.addonManager.getConfig(__name__.split(".")[0]) or {}
            if cfg.get("enable_clipboard_clearing", True) and is_called_from_target_addon():
                if not is_called_from_restore_clipboard():
                    current_mime = self.mimeData()
                    backup = clone_mime_data(current_mime)
                    companion_logger.log(f"[Clipboard Patch] Intercepted prompt copy (len={len(text)}). Applying privacy hints and scheduling restore.")
                    schedule_clipboard_restore(text, backup)
                    
                    private_mime = create_private_mime_data(text)
                    if mode is None:
                        return original_setMimeData(self, private_mime)
                    return original_setMimeData(self, private_mime, mode)
                else:
                    companion_logger.log(f"[Clipboard Patch] Skipping restore scheduling for target addon's own restore_clipboard call.")
                
            if mode is None:
                return original_setText(self, text)
            return original_setText(self, text, mode)
        
        QClipboard.setText = patched_setText

        original_setMimeData = QClipboard.setMimeData
        def patched_setMimeData(self, mime_data, mode=None):
            cfg = mw.addonManager.getConfig(__name__.split(".")[0]) or {}
            if cfg.get("enable_clipboard_clearing", True) and is_called_from_target_addon() and mime_data.hasText():
                if not is_called_from_restore_clipboard():
                    text = mime_data.text()
                    current_mime = self.mimeData()
                    backup = clone_mime_data(current_mime)
                    companion_logger.log(f"[Clipboard Patch] Intercepted prompt mime copy (len={len(text)}). Applying privacy hints and scheduling restore.")
                    schedule_clipboard_restore(text, backup)
                    
                    private_mime = create_private_mime_data(text)
                    for fmt in mime_data.formats():
                        if fmt not in ["text/plain", "text/html", "x-kde-passwordManagerHint", "ExcludeClipboardContentFromMonitorProcessing"]:
                            private_mime.setData(fmt, mime_data.data(fmt))
                            
                    if mode is None:
                        return original_setMimeData(self, private_mime)
                    return original_setMimeData(self, private_mime, mode)
                else:
                    companion_logger.log(f"[Clipboard Patch] Skipping restore scheduling for target addon's own restore_clipboard mime call.")
                
            if mode is None:
                return original_setMimeData(self, mime_data)
            return original_setMimeData(self, mime_data, mode)
        
        QClipboard.setMimeData = patched_setMimeData
        QClipboard._companion_patched = True
        companion_logger.log("[Clipboard Patch] Successfully installed QClipboard backup/restore hooks with privacy hints.")
